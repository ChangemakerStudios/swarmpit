...
public class CouchDbRegistryRepository(CouchDbClient db) : IRegistryRepository
{
    public async Task<List<Registry>> GetAllAsync()
    {
        var docs = await db.FindDocsAsync("registry");
    ...
    public async Task<List<Registry>> GetByTypeAsync(string registryType)
    {
        ...
        var docs = await db.FindDocsAsync("registry", selector);
        return docs.Select(MapRegistry).ToList();
    }

    public async Task<Registry?> GetByIdAsync(string id)
    {
        var doc = await db.GetDocAsync(id);
        return doc != null ? MapRegistry(doc) : null;
    }

    public async Task<RegistryCreateResult> CreateAsync(Registry registry)
    {
        var response = await db.CreateDocAsync(new
        {
            type = "registry",
            name = registry.Name,
        ...
        return new RegistryCreateResult { Id = response.Id, Rev = response.Rev };
    }

    public async Task UpdateAsync(string id, string rev, Registry registry)
    {
        await db.UpdateDocAsync(id, rev, new
        {
            type = "registry",
            name = registry.Name,
            registryType = registry.RegistryType,
    ...
    {
        await db.DeleteDocAsync(id, rev);
    }
