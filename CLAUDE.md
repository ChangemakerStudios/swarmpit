...
- `.github/workflows/build.yml`: on push/PR to master/develop. Builds uberjar, pushes multi-arch docker image to GHCR (`ghcr.io/changemakerstudios/swarmpit`) tagged with GitVersion-computed semver, plus `:latest` on master. multi-arch build (amd64/arm64/armv7/armv5).
- `.github/workflows/release.yml`: triggered by version tag. builds, pushes tagged image, creates github release with jar.
